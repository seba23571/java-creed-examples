...
public class VideoPlayerApplication extends JFrame implements View {
  ...
  private JComboBox<VideoSpeed> playerSpeedComboBox;
  ...
  @Override
  public VideoSpeed getPlayerSpeed() {
    return (VideoSpeed) playerSpeedComboBox.getSelectedItem();
  }
  ...
  public void init() {
    ...
    setTitle("Change the Video Speed with VLC");
  ...
  private void initComponents() {
    ...
    final JPanel controlsPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 2, 2));
    add(controlsPanel, BorderLayout.SOUTH);

    controlsPanel.add(new JButton(presenter.getOpenAction()));
    controlsPanel.add(new JButton(presenter.getStopAction()));
    controlsPanel.add(new JLabel("Player Speed"));

    playerSpeedComboBox = new JComboBox<>(VideoSpeed.values());
    playerSpeedComboBox.setSelectedItem(VideoSpeed.NORMAL);
    playerSpeedComboBox.addActionListener(presenter.getChangeSpeedAction());
    controlsPanel.add(playerSpeedComboBox);
  }
